...
def _read_jsonl_from_s3(s3_key: str) -> pd.DataFrame:
    ...
    try :
        ...
        logger.error(f"Failed to read jsonl from s3 :{e}")
...
def _read_partitioned_parquet_from_s3(s3_prefix: str) -> pd.DataFrame:
    ...
    try : 
        ...
        logger.error(f"Failed to read partitioned parquet from s3 :{e}")
...
def extract_order_line_items() -> pd.DataFrame:
    ...
    try : 
        ...
        logger.error(f"Failed to extract order line items :{e}")
